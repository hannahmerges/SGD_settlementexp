...
## Make kriging plot with maps for just silicate 
...
# Pivot data to long form (if needed)
nData <- nutrients %>%
  pivot_longer(cols = 5:19, names_to = "Parameters", values_to = "Values")

# Kriging function for a single parameter
Krig_function <- function(dat_in = data, Lat = "lat", Lon = "lon", Param = "Values", poly) {
  ...
  # Perform kriging
  krig1 <- kriging(x, y, z, pixels = 500, polygons = poly, lags = 3)  # Adjust pixels as needed
  krig2 <- krig1$map
  
  return(krig2)
}

# Safely handle missing values without breaking the code
...
V_krig_map <- function(datakrig = preds) {
  ...
  ggmap(VarariBaseMap) +
    ...
    ggtitle("Varari Kriging")
}

# Kriging for a single parameter (e.g., Silicate_umolL)
Varari_kriging <- nData %>%
  filter(Location == "Varari") %>%
  filter(CowTagID != "VSEEP") %>%  # Exclude VSEEP
  filter(Parameters == 'Silicate_umolL') %>%  # Select only one parameter
  droplevels() %>%
  mutate(preds = Krig_function_safe(dat_in = ., poly = V_kml)) %>%  # Run kriging for the selected parameter
  mutate(plots = map(preds, ~ggmap(VarariBaseMap) +
...
## Spatial Kriging Map 
- commented out most of Danielle's code because only need singluar kriging plot for silicate variable 
