...
## use these dfs from github for water chem measuremnts at tile locations 
...
## calcualted pH data using Tris calibrations from settlement experiment 
```
...
#### join dfs 
-  join Danielle's percent cover data with my personal recruitment count data from tiles 
-  then join that new df with averaged water chem data from the tile location in situ 
```{r}
...
-  may not be necessary, left comments out 
```{r}
...
full_envdataRecruits_avgseason2 <- full_envdataRecruits_avgseason %>% 
  ...
  filter(CowTagID!="CSEEP", CowTagID!="VSEEP", CowTagID!="Reef_Ambient_1", CowTagID!="Reef_Ambient_2", CowTagID!="Reef_Ambient_3", CowTagID!="Reef_SGD_1", CowTagID!="Reef_SGD_2", CowTagID!="Reef_SGD_3", CowTagID!="Sand_Ambient_1", CowTagID!="Sand_Ambient_2", CowTagID!="Sand_Ambient_3", CowTagID!="Sand_SGD_1", CowTagID!="Sand_SGD_2", CowTagID!="Rockwall_SGD_1", CowTagID!="Rockwall_SGD_2", CowTagID!="Rockwall_SGD_3")
...
-  tidying up the percent cover, getting rid of extra information, pivot to long format to set up in orientation that I can use the data with 
-  created PercentCover3 df which has the percent cover per rebar location at site of dead coral, live coral, rubble, and sand 
-  also created PercentCover_BinomialSubstrate df which separates categories into sand vs hard substrate s
```{r}
PercentCover1 <- PercentCover %>% 
  dplyr::select(!c("del15N":"N_percent", "adj_CT_depth_cm", "meanRugosity"))

PercentCover2 <- PercentCover1 %>% 
  pivot_longer(cols = LiveCoral:Sand, names_to = "comm_type", values_to = "percent_cover") %>% 
  group_by(comm_type, CowTagID, Location) %>% 
  dplyr::summarize(percent_total=sum(percent_cover, na.rm=TRUE)) 
...
PercentCover3 <- PercentCover3 %>% 
  ...
  write_csv(here::here("Data", "Percentcover3.csv"))
...
-  this section calculates all the mins, maxes, CVs, and means for the averaged water chem data 
...
AllChemDataSeasonAvg_calculatedvalues <- full_envdataRecruits_avgseason %>% 
  group_by(CowTagID) %>% 
  dplyr::summarize(sal_avg = mean(Salinity), 
...
AllChemDataSeasonAvg_calculatedvalues2 <- full_envdataRecruits_avgseason %>% 
  group_by(CowTagID) %>% 
  dplyr::summarize(sal_avg = mean(Salinity, na.rm=TRUE), 
...
RecruitmentCounts2.1 <- RecruitmentCounts %>% 
  ...
  dplyr::summarise(sum_total = sum(Pocilloporidae+Poritidae+Acroporidae+Other), 
...
## does not include substrate type and THEREFORE includes C14 
CommComp_recruit_seasonavg_envdata <- RecruitmentCounts2.1 %>% 
  left_join(AllChemDataSeasonAvg_calculatedvalues2) %>% 
  write_csv(here::here("Data", "CommComp_recruit_seasonavg_envdata.csv"))

  

## DOES INCLUDE substrate type/cover
CommComp_recruit2 <- PercentCover_BinomialSubstrate %>% 
  left_join(RecruitmentCounts2.1, join_by(CowTagID, Location)) ### THIS DOES NOT HAVE C14!!! 

CommComp_recruit_seasonavg_envdata2 <- CommComp_recruit2 %>% 
  left_join(AllChemDataSeasonAvg_calculatedvalues2, join_by(CowTagID)) %>% 
  write_csv(here::here("Data", "CommComp_recruit_seasonavg_envdata2.csv"))
...
CommComp_recruit_seasonAvg_envdata_longMAX <- CommComp_recruit_seasonavg_envdata %>% 
  ...
  dplyr::select(!c("sal_avg":"ammonia_CV", "value")) %>% 
  write_csv(here::here("Data", "longMAX.csv"))
...
CommComp_recruit_seasonAvg_envdata_longMIN <- CommComp_recruit_seasonavg_envdata %>%
  ...
  dplyr::select(!c("sal_avg":"ammonia_CV", "value")) %>% 
  write_csv(here::here("Data", "longMIN.csv"))
...
CommComp_recruit_seasonAvg_envdata_longMEAN <- CommComp_recruit_seasonavg_envdata %>% 
  ...
  dplyr::select(!c("sal_max":"ammonia_CV", "value")) %>% 
  write_csv(here::here("Data", "longMEAN.csv"))
...
CommComp_recruit_seasonAvg_envdata_longCV <- CommComp_recruit_seasonavg_envdata %>% 
  ...
  dplyr::select(!c("sal_avg":"ammonia_min", "value")) %>% 
  write_csv(here::here("Data", "longCV.csv"))
...
### calculate CV, mean, max, min of recruit totals per CowTagID ^^^^^^ (as of May 10th, 2024 - code was working)
-  also log transformed the recruit totals (can do within plot or create new column) 
...
### run STATISTICAL models using MEAN values for EACH env parameter (from August and March data), includes silicate, NN, phosphate, etc
...
#### these are extra plots for MIN VALUES only 
```{r}
...
### compare live and dead recruits (last updated May 14th, 2024) 
```{r}
...
ggsave(here::here("Outputs","RecruitmentTiles", "Results", "Prelim", "Dead_vs_Alive_stacked.jpg"), 
       width=20, height=15)
...
CommComp_recruit_envdata_pivot_edit <- CommComp_recruit_envdata_pivot %>% 
  dplyr::select(!c(sal_max:ammonia_CV))
...
-  not really using and haven't cleaned, but kept as back-up 
